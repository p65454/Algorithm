...
#print(type(snake[1][0]), type(snake[1][1]))
...
def direction(t, dir):
    if direction_data[d] == 'D':  # 방향이 오른쪽일 때
        if dir == 3:
            dir = 0
        else:
            dir += 1
    ...
    if cnt == snake_direction[d][0]:
        if direction_data[d] == 'D':  # 방향이 오른쪽일 때(D)
            ...
            if dir == 0:
                ...
                dir -= 1
        if d < len(snake_direction)-1:
            d += 1
        print(f'dir = {dir}')
    #print(f'snake_direction[d][0] = {snake_direction[d][0]}')
print(f'final_cnt = {cnt}')
# 6
# 3
# 3 4
# 2 5
