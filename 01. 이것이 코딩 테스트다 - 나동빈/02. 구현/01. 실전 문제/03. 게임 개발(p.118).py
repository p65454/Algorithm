...
result = 1
move = [(-1, 0), (0, 1), (1, 0), (0, -1)]  # 리스트의 0:북쪽, 1:동쪽, 2:남쪽, 3:서쪽
cur_pos = [(a, b)]
check = 0
...
while True:
    for _ in range(4):
        ...
        next_pos = (a + move[d][0], b + move[d][1])

        if (board[a+move[d][0]][b+move[d][1]] == 0) and (next_pos not in cur_pos):  #해당 방향의 다음 칸이 육지이면서, 가보지 않은 칸일 때
            a += move[d][0]
            b += move[d][1]
            result += 1
            cur_pos.append((a, b))
